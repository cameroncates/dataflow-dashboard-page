...
**Live demo**: [https://modern-board.netlify.app/](https://modern-board.netlify.app/)
...
- **Custom Time Range dropdown**: replaced the native `<select>` with a lightweight popover component for a more modern UI while keeping keyboard support and the same `TimeRange` values.
- **Local icon assets**: dashboard “source” icons and the sidebar logo are served from `public/icons/` to avoid external dependencies and keep the app runnable offline.
...
- **More test coverage with E2Es**:  add a small set of E2E tests (e.g., Playwright) for sidebar + core flows.
...
Around **4 hours** building this dashboard app.
